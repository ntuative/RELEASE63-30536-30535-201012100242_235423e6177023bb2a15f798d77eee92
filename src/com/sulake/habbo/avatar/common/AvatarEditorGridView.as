package com.sulake.habbo.avatar.common
{
   ...
   public class AvatarEditorGridView
   {
      ...
      public function AvatarEditorGridView(param1:IAvatarEditorCategoryModel, param2:String, param3:IHabboWindowManager, param4:IAssetLibrary)
      {
         ...
         if(this._view != null)
         {
            this.var_601 = this._view.findChildByName("part_thumbs") as IItemGridWindow;
            this.var_484 = new Array();
            this.var_484.push(this._view.findChildByName("palette0") as IItemGridWindow);
            this.var_484.push(this._view.findChildByName("palette1") as IItemGridWindow);
         }
      }
      ...
      public function initFromList() : void
      {
         ...
         var _loc5_:* = null;
         var _loc6_:* = null;
         var _loc1_:CategoryData = this.var_62.getCategoryData(this.var_1363);
         if(!_loc1_)
         {
            return;
         }
         this.var_601.removeGridItems();
         for each(_loc2_ in _loc1_.parts)
         {
            if(_loc2_)
            {
               this.var_601.addGridItem(_loc2_.view);
               _loc2_.view.procedure = this.partEventProc;
               if(_loc2_.isSelected)
               {
                  this.method_3(_loc2_.colorLayerCount);
               }
            }
         }
         _loc3_ = 0;
         while(_loc3_ < this.const_1170)
         {
            _loc4_ = _loc1_.getPalette(_loc3_);
            _loc5_ = this.var_484[_loc3_] as IItemGridWindow;
            if(!(!_loc4_ || !_loc5_))
            {
      ...
      {
         var _loc3_:IWindow = this.var_601.getGridItemAt(param1);
         if(!_loc3_)
         {
            return;
         }
      ...
      private function partEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         {
            _loc3_ = this.var_601.getGridItemIndex(param1.window);
            this.var_62.selectPart(this.var_1363,_loc3_);
         }
      }
      ...
      private function paletteEventProc(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowMouseEvent.const_38)
         {
            ...
            _loc7_ = 0;
            while(_loc7_ < this.const_1170)
            {
               if(this.var_484.length > _loc7_)
               {
                  _loc4_ = this.var_484[_loc7_] as IItemGridWindow;
                  _loc5_ = _loc4_.getGridItemIndex(_loc6_);
                  if(_loc5_ > -1)
                  {
                     this.var_62.selectColor(this.var_1363,_loc5_,_loc7_);
                     return;
                  }
               }
               _loc7_++;
