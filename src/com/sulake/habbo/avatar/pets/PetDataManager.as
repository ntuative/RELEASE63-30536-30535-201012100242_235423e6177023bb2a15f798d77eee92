package com.sulake.habbo.avatar.pets
{
   ...
   public class PetDataManager implements IPetDataManager
   {
      ...
      public function init() : void
      {
         ...
         for each(_loc2_ in _loc1_)
         {
            if(_loc2_ != null)
            {
               ...
               if(this.var_396.getValue(_loc4_) == null)
               {
                  this.var_396.add(_loc4_,new PetData(_loc3_));
               }
            }
         }
      }
      ...
      {
         var _loc3_:IPetData = this.var_396.getValue(param1) as IPetData;
         if(_loc3_ == null)
         {
      ...
      {
         var _loc4_:PetData = this.var_396.getValue(param1);
         if(_loc4_ == null)
         {
      ...
      public function populateFigureData(param1:XML) : void
      {
         ...
         speciesKeys = this.var_396.getKeys();
         for each(key in speciesKeys)
         {
            petData = this.var_396.getValue(key);
            if(petData != null)
            {
               ...
               for each(breed in petData.breeds)
               {
                  ...
                  for each(targetSetName in targetSets)
                  {
                     ...
                     {
                        figurePart = figureTargetSet.const_542.part.(@id == petData.species).(hasOwnProperty("palettemapid") && @palettemapid == breed.patternId)[0];
                        if(figurePart == null)
                        {
                        }
      ...
      public function assetsReady(param1:int, param2:String, param3:IPetDataListener = null) : Boolean
      {
         ...
         {
            if(this.var_1368.indexOf(_loc6_) == -1)
            {
               _loc7_ = false;
