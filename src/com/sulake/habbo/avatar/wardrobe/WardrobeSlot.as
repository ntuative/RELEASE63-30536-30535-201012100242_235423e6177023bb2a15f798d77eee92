package com.sulake.habbo.avatar.wardrobe
{
   ...
   public class WardrobeSlot implements IOutfit, IAvatarImageListener
   {
      ...
      private function createView() : void
      {
         ...
         this.var_307 = this._view.findChildByName("image") as IBitmapWrapperWindow;
      }
      ...
      public function updateView() : void
      {
         ...
         if(this.var_562 && this.var_1266)
         {
            _loc5_ = this._controller.avatarRenderManager.createAvatarImage(this.figure,AvatarScaleType.const_83,this.var_827,this);
            if(_loc5_)
            {
               _loc5_.setDirection(AvatarSetType.const_32,parseInt(FigureData.const_816));
         ...
         if(this.var_307)
         {
            if(this.var_307.bitmap)
            {
               this.var_307.bitmap.dispose();
            }
            this.var_307.bitmap = new BitmapData(this.var_307.width,this.var_307.height,true,0);
            _loc7_ = (this.var_307.width - _loc1_.width) / 2;
            _loc8_ = (this.var_307.height - _loc1_.height) / 2;
            this.var_307.bitmap.draw(_loc1_,new Matrix(1,0,0,1,_loc7_,_loc8_));
         }
         if(_loc2_)
         {
            _loc1_.dispose();
      ...
      private function eventHandler(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         switch(param2.name)
         {
            case "set_button":
               ...
               this.var_827 = this._controller.gender;
               this._controller.handler.saveWardrobeOutfit(this.var_1580,this);
               this.updateView();
               break;
            case "get_button":
               if(this.var_562)
               {
                  this._controller.loadAvatarInEditor(this.var_562,this.var_827,this._controller.clubMemberLevel);
               }
         }
      }
