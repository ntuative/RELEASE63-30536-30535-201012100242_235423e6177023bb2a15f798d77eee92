package com.sulake.habbo.tracking
{
   import com.sulake.habbo.configuration.IHabboConfigurationManager;
   
   public class ToolbarClickTracker
   {
       
      
      private var var_1172:IHabboTracking;
      
      private var var_1801:Boolean = false;
      
      private var var_2577:int = 0;
      
      private var var_1823:int = 0;
      
      public function ToolbarClickTracker(param1:IHabboTracking)
      {
         super();
         this.var_1172 = param1;
      }
      
      public function dispose() : void
      {
         this.var_1172 = null;
      }
      
      public function configure(param1:IHabboConfigurationManager) : void
      {
         this.var_1801 = Boolean(parseInt(param1.getKey("toolbar.tracking.enabled","1")));
         this.var_2577 = parseInt(param1.getKey("toolbar.tracking.max.events","100"));
      }
      
      public function track(param1:String) : void
      {
         if(!this.var_1801)
         {
            return;
         }
         ++this.var_1823;
         if(this.var_1823 <= this.var_2577)
         {
            this.var_1172.track("toolbar",param1);
         }
      }
   }
}
