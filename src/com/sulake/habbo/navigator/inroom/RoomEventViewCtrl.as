package com.sulake.habbo.navigator.inroom
{
   ...
   public class RoomEventViewCtrl
   {
      ...
      private var var_1736:ITextWindow;
      ...
      {
         this.var_880.visible = false;
         this.var_1735.visible = true;
         this.var_2399.text = this._navigator.getText("roomevent_type_" + param1.eventType);
         this._eventNameInput.setText(param1.eventName);
         this.var_716.setText(param1.eventDescription);
         this.var_838.setText(this.getTagFromEvent(param1,1));
         this.var_840.setText(this.getTagFromEvent(param1,2));
         this.var_1348.visible = false;
         this.var_1347.visible = true;
         this.var_1346.visible = true;
      ...
      private function onCreateButtonClick(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         var _loc5_:String = this.var_716.getText();
         var _loc6_:Array = new Array();
         this.addTag(_loc6_,this.var_838.getText());
         this.addTag(_loc6_,this.var_840.getText());
      ...
      private function isMandatoryFieldsFilled() : Boolean
      {
         ...
         if(!this.var_716.checkMandatory(this._navigator.getText("navigator.eventsettings.descerr")))
         {
            return false;
         }
         return true;
      ...
      private function prepareWindow(param1:IWindowContainer) : void
      {
         if(this._window == null)
         {
            ...
            this.var_1735 = IWindowContainer(this.find("event_type_disabled_cont"));
            this.var_2399 = ITextWindow(this.find("event_type_disabled_text"));
            this.var_1734 = ITextWindow(this.find("create_caption"));
            this.var_1736 = ITextWindow(this.find("edit_caption"));
            this.var_1348 = IButtonWindow(this.find("create_button"));
            this.var_1347 = IButtonWindow(this.find("edit_button"));
            this.var_2400 = IButtonWindow(this.find("cancel_button"));
            this.var_1346 = IButtonWindow(this.find("end_button"));
            this.var_880 = IDropMenuWindow(this.find("event_type"));
            this.prepareEventTypes();
            Util.setProcDirectly(this.var_1348,this.onCreateButtonClick);
            Util.setProcDirectly(this.var_1347,this.onEditButtonClick);
            Util.setProcDirectly(this.var_1346,this.onEndButtonClick);
            Util.setProcDirectly(this.var_2400,this.onCancelButtonClick);
            this._eventNameInput = new TextFieldManager(this._navigator,this.getInput("event_name"),25);
            this.var_716 = new TextFieldManager(this._navigator,this.getInput("event_desc"),100);
            this.var_838 = new TextFieldManager(this._navigator,this.getInput("event_tag_1"),25);
            this.var_840 = new TextFieldManager(this._navigator,this.getInput("event_tag_2"),25);
