package com.sulake.habbo.widget.memenu
{
   ...
   public class EffectView
   {
      ...
      private var var_156:IWidgetAvatarEffect;
      ...
      private function onUpdate(param1:Event = null) : void
      {
         ...
         {
            if(this.var_156.isActive)
            {
               _loc2_ = this.var_156.secondsLeft / Number(this.var_156.duration);
               this.var_797.width = _loc2_ * this.var_480;
            }
            else
            {
               this.var_797.width = 0;
               this.var_178.stop();
            }
            this.setTimeLeft();
         }
      ...
      private function setTimeLeft() : void
      {
         ...
         {
            this.var_798.caption = "${widgets.memenu.effects.active.timeleft}";
            _loc1_ = this.var_798.text;
            if(this.var_156.isActive)
            {
               _loc2_ = this.var_156.secondsLeft;
               ...
               this.var_798.text = _loc1_;
            }
            else
            {
               this.var_798.caption = "${widgets.memenu.effects.activate}";
            }
         }
      }
      ...
      private function createWindow(param1:String) : void
      {
         ...
         var _loc7_:* = null;
         var _loc3_:String = "";
         if(this.var_156.isInUse)
         {
            _loc3_ = "memenu_effect_selected";
         }
         else if(this.var_156.isActive)
         {
         ...
         else
         {
            ...
            this.var_796 = this._window.findChildByName("effect_hilite");
            this.var_796.visible = false;
         }
         this.var_798 = this._window.findChildByName("time_left") as ITextWindow;
         this.setTimeLeft();
         this.var_797 = this._window.findChildByName("loader_bar");
         if(this.var_797 != null)
         {
            this.var_480 = this.var_797.width;
            this.var_178.start();
            this.onUpdate();
         }
         if(this.var_156.icon)
         {
            this.setElementBitmap("effect_icon",this.var_156.icon);
         }
      }
