package com.sulake.habbo.widget.roomqueue
{
   ...
   public class RoomQueueWidget extends RoomWidgetBase
   {
      ...
      override public function registerUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.addEventListener(RoomWidgetRoomQueueUpdateEvent.const_492,this.onQueueStatus);
      ...
      override public function unregisterUpdateEvents(param1:IEventDispatcher) : void
      {
         ...
         param1.removeEventListener(RoomWidgetRoomQueueUpdateEvent.const_492,this.onQueueStatus);
      }
      ...
      private function onQueueStatus(param1:RoomWidgetRoomQueueUpdateEvent) : void
      {
         ...
         if(param1.isActive)
         {
            this.var_1483 = param1.type;
            this.var_1215 = false;
            this.var_970 = param1.position;
         }
         else
         {
            this.var_1215 = true;
         }
         this.var_1992 = param1.hasHabboClub;
         this.var_1484 = param1.isClubQueue;
         localizations.registerParameter("room.queue.position","position",this.var_970.toString());
         localizations.registerParameter("room.queue.position.hc","position",this.var_970.toString());
         localizations.registerParameter("room.queue.spectator.position","position",this.var_970.toString());
         localizations.registerParameter("room.queue.spectator.position.hc","position",this.var_970.toString());
         this.showInterface();
      }
      ...
      private function showInterface() : void
      {
         ...
         {
            switch(this.var_1483)
            {
               case RoomWidgetRoomQueueUpdateEvent.const_342:
                  _loc1_.caption = !!this.var_1484 ? "${room.queue.position.hc}" : "${room.queue.position}";
                  _loc2_.caption = "${room.queue.spectatormode}";
                  _loc3_.visible = this.var_1215;
                  break;
               case RoomWidgetRoomQueueUpdateEvent.const_492:
                  _loc1_.caption = !!this.var_1484 ? "${room.queue.spectator.position.hc}" : "${room.queue.spectator.position}";
                  _loc2_.caption = "${room.queue.back}";
                  _loc3_.visible = false;
            }
            _loc2_.visible = this.var_1215;
         }
         var _loc4_:IWindowContainer = this._window.findChildByName("club_container") as IWindowContainer;
         if(_loc4_ != null)
         {
            _loc4_.visible = !this.var_1992;
         }
      ...
      private function exitQueue(param1:WindowMouseEvent) : void
      {
         ...
         var _loc2_:RoomWidgetRoomQueueMessage = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_797);
      ...
      private function changeQueue(param1:WindowMouseEvent) : void
      {
         ...
         if(this.var_1483 == RoomWidgetRoomQueueUpdateEvent.const_342)
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.CHANGE_TO_SPECTATOR_QUEUE);
         }
         else
         {
            _loc2_ = new RoomWidgetRoomQueueMessage(RoomWidgetRoomQueueMessage.const_628);
         }
         messageListener.processWidgetMessage(_loc2_);
         this.removeWindow();
      }
