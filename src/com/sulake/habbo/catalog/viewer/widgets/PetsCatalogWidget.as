package com.sulake.habbo.catalog.viewer.widgets
{
   ...
   public class PetsCatalogWidget extends CatalogWidget implements ICatalogWidget, IPetDataListener, IPetImageListener
   {
      ...
      override public function init() : void
      {
         ...
         this.var_787 = this.getPetTypeIndexFromProduct(_loc5_.localizationId);
         this.var_295 = [];
         this.var_527 = [];
         if(page && false && page.viewer.catalog)
         {
            ...
            {
               _loc9_ = _loc8_.avatarRenderManager.petDataManager.getPetData(this.var_787,this);
               if(_loc9_)
               {
                  this.var_527 = _loc9_.sellableBreeds;
                  this.var_295 = _loc9_.colors;
         ...
         var _loc6_:int = 0;
         while(_loc6_ < this.var_527.length)
         {
            _loc10_ = this.var_527[_loc6_] as IBreed;
            _loc3_.push(_loc4_.getKey(this.getRaceLocalizationKey(this.var_787,_loc10_.id),this.getRaceLocalizationKey(this.var_787,_loc10_.id)));
            _loc6_++;
         }
         this._offers.add(this.var_787,_loc5_);
         ...
         events.addEventListener(WidgetEvent.const_1064,this.onApproveNameResult);
      ...
      public function onDropMenuEvent(param1:WindowEvent, param2:IWindow) : void
      {
         ...
         if(param1.type == WindowEvent.const_58)
         {
            ...
            if(_loc3_ >= this.var_527.length)
            {
               return;
            }
            this.var_1482 = _loc3_;
            this.updateImage();
         }
      }
      ...
      private function onColourIndex(param1:CatalogWidgetColourIndexEvent) : void
      {
         ...
         this.var_430 = param1.index;
         if(this.var_430 < 0 || this.var_430 > this.var_295.length)
         {
            this.var_430 = 0;
         }
         this.updateImage();
      ...
      private function updateImage() : void
      {
         ...
         if(this.var_430 >= 0 && this.var_430 < this.var_295.length)
         {
            _loc7_ = this.var_295[this.var_430] as IPetColor;
            if(_loc7_ != null)
            {
               _loc2_ = uint(_loc7_.rgb);
            }
         }
         var _loc3_:IBreed = this.var_527[this.var_1482] as IBreed;
         if(!_loc3_)
         {
            return;
         }
         var _loc4_:IAvatarImage = (page.viewer.catalog as HabboCatalog).avatarRenderManager.createPetImage(this.var_787,_loc3_.id,_loc2_,AvatarScaleType.const_47,this);
         if(_loc4_ == null)
         {
            return;
         }
         _loc4_.initActionAppends();
         _loc4_.appendAction(AvatarAction.const_271,AvatarAction.const_676);
      ...
      private function getPurchaseParameters() : String
      {
         ...
         if(this.var_430 >= this.var_295.length)
         {
            return "";
         }
         var petColor:IPetColor = this.var_295[this.var_430] as IPetColor;
         var color:uint = petColor.rgb;
         var breed:IBreed = this.var_527[this.var_1482] as IBreed;
