   ...
   public class TextSkinRenderer extends SkinRenderer
   {
      ...
      override public function draw(param1:IWindow, param2:IBitmapDrawable, param3:Rectangle, param4:uint, param5:Boolean) : void
      {
         ...
         if(param2 != null)
         {
            ...
            this.var_925.tx = _loc6_.margins.left;
            this.var_925.ty = _loc6_.margins.top;
            if(_loc7_ == TextFieldAutoSize.RIGHT)
            {
               this.var_925.tx = Math.floor(param1.width - _loc8_.width);
            }
            else if(_loc7_ == TextFieldAutoSize.CENTER)
            {
               this.var_925.tx = Math.floor(param1.width / 2 - _loc8_.width / 2);
            }
            BitmapData(param2).draw(_loc8_,this.var_925,null,null,null,false);
         }
      }
